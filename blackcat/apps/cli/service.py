...
from blackcat.apps.protocol import app_manifest, compact_dict
...
class CliAppManager:
    ...
    def _manifest_source(self, app: dict[str, Any]) -> str:
        ...
        if source == "extensions":
            return "blackcat-extension"
        return f"cli-anything:{source}"

    def _trust_registry(self, app: dict[str, Any]) -> str:
        return "blackcat-extension" if str(app.get("_source") or "") == "extensions" else "cli-anything"
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["blackcat_state_absent", "managed_paths_absent"]
            ),
        })
        return app_manifest(
            app_id=name,
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from blackcat."
        return f"""---
...
Use this skill when the user asks blackcat to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_blackcat_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- blackcat-cli-app-note -->"
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_blackcat_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from blackcat. {entry_point} "
                "is still available because it is managed outside blackcat."
            )
