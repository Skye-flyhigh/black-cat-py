...
def test_shared_and_legacy_paths_remain_global() -> None:
    assert get_cli_history_path() == Path.home() / ".blackcat" / "history" / "cli_history"
    assert get_bridge_install_dir() == Path.home() / ".blackcat" / "bridge"
    assert get_legacy_sessions_dir() == Path.home() / ".blackcat" / "sessions"


def test_workspace_path_is_explicitly_resolved() -> None:
    assert get_workspace_path() == Path.home() / ".blackcat" / "workspace"
...
def test_is_default_workspace_distinguishes_default_and_custom_paths() -> None:
    ...
    assert is_default_workspace(Path.home() / ".blackcat" / "workspace") is True
