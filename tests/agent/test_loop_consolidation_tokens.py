...
async def test_prompt_above_threshold_archives_until_next_user_boundary(tmp_path, monkeypatch) -> None:
    ...
    assert loop.consolidator.archive.called
    archived_chunk = loop.consolidator.archive.call_args.args[0]
...
async def test_consolidation_loops_until_target_met(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("blackcat.utils.tokens.estimate_message_tokens", lambda _m: 100)

    await loop.consolidator.maybe_consolidate_by_tokens(session)

    # Consolidation runs until target met or no safe boundary found
    assert loop.consolidator.archive.await_count >= 1
    assert session.last_consolidated >= 4
...
async def test_consolidation_persists_summary_for_next_prepare_session(tmp_path, monkeypatch) -> None:
    ...
    loop.consolidator.estimate_session_prompt_tokens = mock_estimate  # type: ignore[method-assign]
    monkeypatch.setattr("blackcat.utils.tokens.estimate_message_tokens", lambda _m: 150)
...
async def test_preflight_consolidation_receives_pending_summary(tmp_path) -> None:
    ...
    # maybe_consolidate_by_tokens no longer accepts session_summary parameter
    assert loop.consolidator.maybe_consolidate_by_tokens.await_count == 1
...
async def test_preflight_consolidation_before_llm_call(tmp_path, monkeypatch) -> None:
    ...
    async def track_consolidate(messages, session_summary=None):
