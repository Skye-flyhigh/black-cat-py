...
from blackcat.channels.matrix import (
    MATRIX_HTML_FORMAT,
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("blackcat.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "blackcat.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("blackcat.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "blackcat.channels.matrix.asyncio.create_task", _fake_create_task
    )
