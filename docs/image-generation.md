...
blackcat can generate and edit images through the `generate_image` tool. In the WebUI, users can enable **Image Generation** from the composer, choose an aspect ratio, and keep iterating on generated images inside the same chat.

The feature is disabled by default. Enable it in `~/.blackcat/config.json`, configure a supported image provider, then restart the gateway.
...
> [!TIP]
> Prefer environment variables for API keys. blackcat resolves `${VAR_NAME}` values from the environment at startup.
...
| `tools.imageGeneration.saveDir` | string | `"generated"` | Relative directory under blackcat's media directory for generated artifacts |
...
AIHubMix `gpt-image-2-free` is supported through AIHubMix's unified predictions API. Internally blackcat calls:
...
blackcat supports two Gemini image generation model families via Google's Generative Language API:
...
Zhipu (智谱) `glm-image` model supports text-to-image generation. The API returns temporary image URLs (valid for 30 days); blackcat downloads and re-encodes them as base64 data URLs.
...
Generated images are stored under the active blackcat instance's media directory:

```text
~/.blackcat/media/generated/YYYY-MM-DD/img_<id>.<ext>
~/.blackcat/media/generated/YYYY-MM-DD/img_<id>.json
```
...
```text
A minimal app icon for blackcat: friendly robot head, rounded square, soft blue and white palette, clean vector style, no text
```
