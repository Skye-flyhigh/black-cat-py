...
blackcat discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `blackcat gateway` starts, it scans:

1. Built-in channels in `blackcat/channels/`
2. External packages registered under the `blackcat.channels` entry point group
...
```text
blackcat-channel-webhook/
├── blackcat_channel_webhook/
...
```python
# blackcat_channel_webhook/__init__.py
from blackcat_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# blackcat_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "blackcat-channel-webhook"
version = "0.1.0"
dependencies = ["blackcat-ai", "aiohttp"]

[project.entry-points."blackcat.channels"]
webhook = "blackcat_channel_webhook:WebhookChannel"
...
packages = ["blackcat_channel_webhook"]
```
...
```bash
blackcat channels login <channel_name>
blackcat channels login <channel_name> --force  # re-authenticate
```
...
Built-in channels use Pydantic config models (subclassing `Base` from `blackcat.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `blackcat.config.schema.Base`:
...
Override `default_config()` so `blackcat onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `blackcat-channel-{name}` | `blackcat-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `blackcat_channel_{name}` | `blackcat_channel_webhook` |
