...
from nanobot.channels.utils import extract_markdown_tables
...
class FeishuChannel(BaseChannel):
    ...
    async def _send_impl(self, msg: OutboundMessage) -> None:
        """Feishu-specific send implementation."""
        if not self._client:
        ...
        try:
            ...
            elements = extract_markdown_tables(msg.content)
            card = {
    ...
    def _trim_processed_ids(self) -> None:
        """Trim the dedup cache when it gets too large."""
        if len(self._processed_ids) > self._MAX_PROCESSED_IDS:
            while len(self._processed_ids) > self._TRIM_TO:
