  ...
  it("groups WebUI chats by workspace project while preserving in-project sorting and activity", () => {
    const sessions = [
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/blackcat",
      ...
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/blackcat-bench",
          project_name: "blackcat-bench",
          access_mode: "full",
        },
      }),
    ];
    ...
    const blackcatSection = screen.getByRole("region", { name: "blackcat" });
    const blackcatText = blackcatSection.textContent ?? "";

    expect(screen.getByRole("region", { name: "blackcat-bench" })).toBeInTheDocument();
    expect(within(blackcatSection).getByText("Alpha task")).toBeInTheDocument();
    expect(within(blackcatSection).getByText("Zeta task")).toBeInTheDocument();
    expect(blackcatText.indexOf("Alpha task")).toBeLessThan(blackcatText.indexOf("Zeta task"));
    expect(within(blackcatSection).getByLabelText("Agent running")).toBeInTheDocument();
  ...
  it("keeps default workspace chats in the Chats section instead of a project folder", () => {
    const sessions = [
      session({
        ...
        updatedAt: "2026-05-21T10:00:00Z",
        workspaceScope: {
          project_path: "/Users/me/.blackcat/workspace",
    ...
    render(
      <ChatList
        ...
        onRequestRename={vi.fn()}
        onToggleArchive={vi.fn()}
        defaultWorkspacePath="/Users/me/.blackcat/workspace"
        showTimestamps
      />,
    );

    expect(screen.getByText("Projects")).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "blackcat" })).toBeInTheDocument();
  ...
  it("can collapse a project group and keeps project rename separate from chat titles", async () => {
    ...
    render(
      <ChatList
        ...
        projectNameOverrides={{ "/Users/me/blackcat": "Photos" }}
        collapsedGroups={{ "project:/Users/me/blackcat": true }}
      />,
    );
    ...
    expect(onToggleGroup).toHaveBeenCalledWith("project:/Users/me/blackcat");
    ...
    expect(onNewChatInProject).toHaveBeenCalledWith("/Users/me/blackcat", "Photos");
    ...
    expect(onRequestRenameProject).toHaveBeenCalledWith("/Users/me/blackcat", "Photos");
  });
