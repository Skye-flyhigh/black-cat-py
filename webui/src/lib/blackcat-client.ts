...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "blackcat-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('blackcat_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('blackcat_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("blackcat_debug_ws")?.trim().toLowerCase() ?? "";
